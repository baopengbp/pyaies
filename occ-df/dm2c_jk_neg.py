...
def get_jk(dfobj, dm, hermi=1, with_j=True, with_k=True, direct_scf_tol=1e-13):
    ...
    if getattr(dm, 'mo_coeff', None) is not None:
        ...
        if nset == 1:
            ...
            #print('uuuuuuuuuuuuuuu',mo_occa, mo_occb)
            mo_coeff = numpy.vstack((mo_coeffa, mo_coeffb))
            mo_occ = numpy.vstack((mo_occa, mo_occb))    
        occ_negnum = mo_occ[mo_occ < -occ_thrd].shape[-1]
        if occ_negnum == 0:
            dm_postive = True
        else:
            dm_postive = False
    ...
    if not with_k:
        for eri1 in dfobj.loop():
            ...
            vj += lib.dot(rho, eri1)
    elif dm_postive == True:
        #print('dm_postive == True')
        nmo = mo_occ.shape[-1]
        ...
        if mo_occ.shape[0] * 2 == nset: # handle ROHF DM
            ...
            mo_occa = numpy.array(mo_occ> occ_thrd, dtype=numpy.double)
        ...
        worbo = []
        for k in range(nset):
            orbo.append(numpy.asarray(mo_coeff[k][:,abs(mo_occ[k])>occ_thrd], order='F'))
            worbo.append(orbo[k] * mo_occ[k][abs(mo_occ[k])>1.E-10])
        max_memory = dfobj.max_memory - lib.current_memory()[0]
        blksize = max(4, int(min(dfobj.blockdim, max_memory*.3e6/8/nao**2)))
        buf = numpy.empty((blksize*nao,nao))
        for eri1 in dfobj.loop(blksize):
            ...
            for k in range(nset):
                ...
                if nocc > 0:
                    ...
                    fdrv(ftrans, fmmm,
                         ...
                         (ctypes.c_int*4)(0, nocc, 0, nao),
                         null, ctypes.c_int(0))
                    buf1 = buf1.reshape(naux,nocc,-1)
                    #buf2 =  numpy.einsum('pij,i->pij', buf1, mo_occ[k][abs(mo_occ[k])>occ_thrd])
                    v = mo_occ[k][abs(mo_occ[k])>occ_thrd].reshape(1, -1, 1)
                    buf2 =  ne.evaluate("buf1 * v")
                    vk[k] += numpy.tensordot(buf1, buf2, axes=([0,1],[0,1]))
            t1 = log.timer_debug1('jk', *t1)
