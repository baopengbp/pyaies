...
# modified by Peng Bao <baopeng@iccas.ac.cn>
...
def eval_rho3(mol, ao, mo_coeff, mo_occ, non0tab=None, xctype='LDA',
              verbose=None):
    r'''Calculate the electron density for LDA functional, and the density
    derivatives for GGA functional.  This function has the same functionality
    as :func:`eval_rho` except that the density are evaluated based on orbital
...
def nr_rks(ni, mol, grids, xc_code, dms, relativity=0, hermi=0,
    ...
    if xctype == 'LDA':
        ...
        if mol.atom != ni.atom_old:
            print('\ngridset---r---lda\n')
            for ao, mask, weight, coords \
                    in ni.block_loop_incore(mol, grids, nao, ao_deriv, max_memory):
                print('incore ao, mask, weight, coords, SWITCH_SIZE', SWITCH_SIZE)
                ni.atom_old = mol.atom
        if 0==0:
            aow = numpy.ndarray(ao.shape, order='F', buffer=aow)
...
def nr_rks_fxc(ni, mol, grids, xc_code, dm0, dms, relativity=0, hermi=0,
    ...
    if xctype == 'LDA':
        ...
        for ao, mask, weight, coords \
                in ni.block_loop(mol, grids, nao, ao_deriv, max_memory):
            print('no fxc incore ao, mask, weight, coords, SWITCH_SIZE', SWITCH_SIZE)
...
def nr_rks_fxc_st(ni, mol, grids, xc_code, dm0, dms_alpha, relativity=0, singlet=True,
    ...
    if xctype == 'LDA':
        ...
        for ao, mask, weight, coords \
                in ni.block_loop_incore(mol, grids, nao, ao_deriv, max_memory):
            print('fxc_st incore ao, mask, weight, coords, SWITCH_SIZE', SWITCH_SIZE)
...
class NumInt(object):
    ...
    def block_loop_incore(self, mol, grids, nao=None, deriv=0, max_memory=2000,
                   non0tab=None, blksize=None, buf=None):
        '''Define this macro to loop over grids by blocks.
        '''

        global ao, non0, weight, coords
...
# NOTE to index grids.non0tab, the blksize needs to be the integer multiplier of BLKSIZE
        if blksize is None:
            ...
            blksize = ngrids
            print('\n///////      incore blksize, ngrids',blksize, ngrids)
...
# NOTE to index grids.non0tab, the blksize needs to be the integer multiplier of BLKSIZE
    ...
    def _gen_rho_evaluator_dm2c(self, mol, dms, hermi=0):
    #def _gen_rho_evaluator(self, mol, dms, hermi=0):
        if getattr(dms, 'mo_coeff', None) is not None:
            ...
            mo_occ, mo_coeff = numpy.linalg.eigh(dms)
            mo_coeff = mo_coeff[:,mo_occ>1.E-10]
            mo_occ = mo_occ[mo_occ>1.E-10]

            print('dm2c',mo_coeff.shape,mo_occ.shape,dms.shape)
            if isinstance(dms, numpy.ndarray) and dms.ndim == 2:
                mo_coeff = [mo_coeff]
                mo_occ = [mo_occ]
            nao = mo_coeff[0].shape[0]
    ...
    #def _gen_rho_evaluator_orig(self, mol, dms, hermi=0):
    def _gen_rho_evaluator(self, mol, dms, hermi=0):
        if getattr(dms, 'mo_coeff', None) is not None:
            #TODO: test whether dm.mo_coeff matching dm
            mo_coeff = dms.mo_coeff
