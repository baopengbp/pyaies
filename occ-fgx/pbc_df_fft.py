...
KE_SCALING = getattr(__config__, 'pbc_df_aft_ke_cutoff_scaling', 0.75)
    ...
    # post-HF methods.
def get_jk_occ(self, dm, hermi=1, kpts=None, kpts_band=None,
...
def get_jk_occ_pymp(self, dm, hermi=1, kpts=None, kpts_band=None,
               with_j=True, with_k=True, omega=None, exxdiv=None):
        from pyscf.pbc.df import fft_jk
        import fft_occk
        if omega is not None:  # J/K for RSH functionals
        ...
        if kpts.shape == (3,):
            ...
            if with_k:
                vk = fft_occk.get_k_kpts_occ_pymp(self, dm, hermi, kpts, kpts_band, exxdiv)
            if with_j:
                vj = fft_occk.get_j_kpts(self, dm, hermi, kpts, kpts_band)
        return vj, vk

def get_jk_pymp(self, dm, hermi=1, kpts=None, kpts_band=None,
               with_j=True, with_k=True, omega=None, exxdiv=None):
        from pyscf.pbc.df import fft_jk
        import fft_occk
        if omega is not None:  # J/K for RSH functionals
        ...
        if kpts.shape == (3,):
            ...
            if with_k:
                vk = fft_occk.get_k_kpts_pymp(self, dm, hermi, kpts, kpts_band, exxdiv)
            if with_j:
                vj = fft_occk.get_j_kpts(self, dm, hermi, kpts, kpts_band)
        return vj, vk

def get_jk_opt(self, dm, hermi=1, kpts=None, kpts_band=None,
               with_j=True, with_k=True, omega=None, exxdiv=None):
        from pyscf.pbc.df import fft_jk
        import fft_occk
        if omega is not None:  # J/K for RSH functionals
        ...
        if kpts.shape == (3,):
            ...
            if with_k:
                vk = fft_occk.get_k_kpts_opt(self, dm, hermi, kpts, kpts_band, exxdiv)
            if with_j:
                vj = fft_occk.get_j_kpts(self, dm, hermi, kpts, kpts_band)
        return vj, vk

from pyscf.pbc.df import fft
fft.FFTDF.first_cyc = None
